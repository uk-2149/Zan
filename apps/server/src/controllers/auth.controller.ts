...
export const getWalletStatus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (address) {
      ...
      if (owner) {
        addressStatus =
          owner.id === userId ? "linkedToCurrentUser" : "linkedToAnotherUser";
      }
    }

    res.json({
      linkedWalletAddress: user.walletAddress ?? null,
      addressStatus,
    });
  } catch (err) {
    console.error(err);
...
export const updateWallet = async (req: Request, res: Response) => {
  ...
  if (!walletAddress || !signature || !message) {
    return res
      .status(400)
      .json({ error: "walletAddress, signature and message required" });
  }
  ...
  if (
    ...
    user.walletNonceExpiry < new Date()
  ) {
    return res
      .status(400)
      .json({ error: "Challenge expired — request a new one" });
  }
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "Invalid wallet address or signature" });
  }
