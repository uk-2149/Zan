...
function getShapePoint(step: number, index: number, total: number) {
  ...
  if (step === 0) {
    if (pct < 0.4) {
      ...
      if (index % 2 === 0) {
        return { x: cx + (Math.random() - 0.5) * 350, y: cy + (Math.random() - 0.5) * 4 };
      } else {
        return { x: cx + (Math.random() - 0.5) * 4, y: cy + (Math.random() - 0.5) * 350 };
      }
    }
  }
  ...
  if (step === 1) {
    ...
    if (pct < 0.6) {
      ...
      const t = Math.random();
      return {
        x: (start?.x ?? 0) + ((end?.x ?? 0) - (start?.x ?? 0)) * t + (Math.random() - 0.5) * 4,
        y: (start?.y ?? 0) + ((end?.y ?? 0) - (start?.y ?? 0)) * t + (Math.random() - 0.5) * 4,
      };
    }
  }
          ...
          <div className="w-full lg:w-1/2 py-24 md:py-32">
            {steps.map((step, idx) => (
              ...
              >
                ...
                <div className={`transition-opacity duration-700 ${activeStep === idx ? 'opacity-100' : 'opacity-20'}`}>
                  ...
                  <ul className="flex flex-col gap-4">
                    {step.bullets?.map((bullet: string, bIdx: number) => (
                      <li key={bIdx} className="flex items-center gap-4 text-white/70">
